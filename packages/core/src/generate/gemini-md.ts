// GEMINI.md generator — Gemini CLI instruction file format.
...
// ---------------------------------------------------------------------------
// Rendering helpers (Gemini CLI style: imperative, prescriptive)
// ---------------------------------------------------------------------------
...
function renderConventions(conventions: ConventionProfile[]): string {
  if (conventions.length === 0) {
    return '_No convention data yet. Run `mindr generate gemini-md` after more commits._\n'
  }
  const out: string[] = []
...
export async function generateGeminiMd(
  ...
  lines.push('mindr generate gemini-md')
  lines.push('```')
  lines.push('')
